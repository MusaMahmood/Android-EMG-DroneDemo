...
static void b_sum(const double x[30080], double y[235]);
static void c_power(const double a[30080], double y[30080]);
...
static void hist(const double Y[5], const double X_data[], const int X_size[1],
                 double no_data[], int no_size[2]);
static void histc(const double X[5], const double edges_data[], const int
                  edges_size[2], double N_data[], int N_size[1]);
static double knn(const double tsX[128], const double tX[30080], const double
                  tY[235]);
static double mean(const double x_data[], const int x_size[1]);
static void merge(int idx[235], double x[235], int offset, int np, int nq, int
                  iwork[235], double xwork[235]);
...
static void sort(double x[235], int idx[235]);
...
//
// Arguments    : const double x[30080]
//                double y[235]
// Return Type  : void
//
static void b_sum(const double x[30080], double y[235])
...
//
// Arguments    : const double a[30080]
//                double y[30080]
// Return Type  : void
//
static void c_power(const double a[30080], double y[30080])
                 ...
                 double no_data[], int no_size[2])
{
  int edges_size[2];
  int k;
  double edges_data[236];
  double nn_data[236];
  int nn_size[1];
  edges_size[0] = 1;
  edges_size[1] = (unsigned char)(X_size[0] + 1);
  for (k = 0; k <= X_size[0] - 2; k++) {
    edges_data[k + 1] = X_data[k] + (X_data[k + 1] - X_data[k]) / 2.0;
  }

  edges_data[0] = rtMinusInf;
  edges_data[(unsigned char)(X_size[0] + 1) - 1] = rtInf;
                  ...
                  edges_size[2], double N_data[], int N_size[1])
{
  ...
  int high_i;
  int mid_i;
  N_size[0] = (unsigned char)edges_size[1];
  xind = (unsigned char)edges_size[1];
  for (k = 0; k < xind; k++) {
    N_data[k] = 0.0;
  }
...
//     type = '2norm';
// end
// Arguments    : const double tsX[128]
//                const double tX[30080]
//                const double tY[235]
// Return Type  : double
//
static double knn(const double tsX[128], const double tX[30080], const double
                  tY[235])
{
  ...
  int i2;
  int Uc_size[1];
  static double b_tX[30080];
  static double dv9[30080];
  double n_data[235];
  double b_tY[5];
  int n_size[2];
  boolean_T exitg2;
  memset(&idx[0], 0, 235U * sizeof(int));
  for (k = 0; k <= 232; k += 2) {
    if ((tY[k] <= tY[k + 1]) || rtIsNaN(tY[k + 1])) {
      p = true;
    } else {
      p = false;
    }
  ...
  while (i < 235) {
    khi = i << 1;
    j = 1;
    for (pEnd = 1 + i; pEnd < 236; pEnd = qEnd + i) {
      nb = j;
      b_k = pEnd - 1;
      qEnd = j + khi;
      if (qEnd > 236) {
      ...
      kEnd = qEnd - j;
      while (k + 1 <= kEnd) {
        if ((tY[idx[nb - 1] - 1] <= tY[idx[b_k] - 1]) || rtIsNaN(tY[idx[b_k] - 1]))
        {
          p = true;
        } else {
          p = false;
  ...
  k = 0;
  while ((k + 1 <= 235) && rtIsInf(Uc_data[k]) && (Uc_data[k] < 0.0)) {
    k++;
  }

  b_k = k;
  k = 235;
  while ((k >= 1) && rtIsNaN(Uc_data[k - 1])) {
    k--;
  }

  pEnd = 235 - k;
  while ((k >= 1) && rtIsInf(Uc_data[k - 1]) && (Uc_data[k - 1] > 0.0)) {
    k--;
  }
  ...
  while (b_k + 1 <= khi) {
    ...
    do {
      ...
      if (b_k + 1 > khi) {
        ...
        if ((std::abs(x - Uc_data[b_k]) < eps(x / 2.0)) || (rtIsInf(Uc_data[b_k])
             && rtIsInf(x) && ((Uc_data[b_k] > 0.0) == (x > 0.0)))) {
          p = true;
        } else {
          p = false;
        }
  ...
  Uc_size[0] = i2 + 1;
  for (khi = 0; khi < 235; khi++) {
    for (i = 0; i < 128; i++) {
      b_tX[khi + 235 * i] = tX[khi + 235 * i] - tsX[i];
    }
  }
  ...
  hist(b_tY, Uc_data, Uc_size, n_data, n_size);
  khi = 1;
  i = n_size[1];
  x = n_data[0];
  pEnd = 0;
...
static void sort(double x[235], int idx[235])
{
  ...
  memset(&xwork[0], 0, 235U * sizeof(double));
  nNaNs = -234;
  ib = 0;
  for (k = 0; k < 235; k++) {
    if (rtIsNaN(x[k])) {
  ...
  for (k = 1; k <= i; k++) {
    ib = idx[k - nNaNs];
    idx[k - nNaNs] = idx[235 - k];
    idx[235 - k] = ib;
    x[k - nNaNs] = xwork[235 - k];
    x[235 - k] = xwork[k - nNaNs];
  }
  ...
  if (1 - nNaNs > 1) {
    memset(&iwork[0], 0, 235U * sizeof(int));
    nPairs = (1 - nNaNs) >> 2;
    bLen = 4;
    while (nPairs > 1) {
      if ((nPairs & 1) != 0) {
...
//
// Arguments    : const double rawData[500]
//                const double F[30315]
// Return Type  : double
//
double classifyArmEMG1ch(const double rawData[500], const double F[30315])
{
  double FILT_FULL[500];
  double b_FILT_FULL[500];
  double dWF[250];
  int ixstart;
  double mtmp;
  int ix;
  boolean_T exitg1;
  static const double dv0[250] = { 0.0, 0.00015917615386623618,
  ...
  //  2Hz High Pass:
  filtfilt(rawData, FILT_FULL);
  memcpy(&b_FILT_FULL[0], &FILT_FULL[0], 500U * sizeof(double));
  b_filtfilt(b_FILT_FULL, FILT_FULL);
  memcpy(&dWF[0], &FILT_FULL[125], 250U * sizeof(double));
  ixstart = 1;
  mtmp = dWF[0];
  if (rtIsNaN(dWF[0])) {
    ix = 2;
  ...
  welch_psd(dWF, dv0, PSD, unusedU0);

  dv1[0] = rms(dWF);
  sig_rms_pad_fixed(dWF, dv2);
  dv1[1] = trapz(dv2);
  dv1[2] = mtmp;
  memcpy(&dv1[3], &PSD[0], 125U * sizeof(double));
  return knn(dv1, *(double (*)[30080])&F[0], *(double (*)[235])&F[30080]);
}
...
// Return Type  : void
//
void classifyArmEMG1ch_initialize()
{
  rt_InitInfAndNaN(8U);
}
...
// Return Type  : void
//
void classifyArmEMG1ch_terminate()
{
  // (no terminate code required)
}

//
// File trailer for classifyArmEMG1ch.cpp
//
// [EOF]
//
